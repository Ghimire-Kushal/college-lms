...
function AttendanceRing({ pct }) {
  ...
  const color = pct >= 75 ? '#0f766e' : pct >= 50 ? '#d97706' : '#2563eb';
  return (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="rounded-2xl p-5 sm:p-6 border shadow-sm flex flex-col items-center justify-center"
          ...
          <div className="mt-4 flex gap-6 text-center">
            <div>
              <p className="text-xl font-bold" style={{ color: '#0f766e' }}>{data?.presentClasses ?? 0}</p>
              <p className="text-[11px]" style={{ color: subClr }}>Present</p>
            </div>
            <div>
              <p className="text-xl font-bold" style={{ color: '#2563eb' }}>{(data?.totalClasses ?? 0) - (data?.presentClasses ?? 0)}</p>
          ...
          <div className="space-y-2.5">
            {data?.upcomingAssignments?.length === 0 && (
              <div className="py-8 text-center" style={{ color: subClr }}>
                <CheckCircle size={28} className="mx-auto mb-2 opacity-30" style={{ color: '#0f766e' }} />
